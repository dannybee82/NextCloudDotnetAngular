...
namespace ServiceLayer.Services
{
    ...
    public class NextCloudService : INextCloudService
    {
        ...
        public async Task<FileDownloadDto> DownloadFile(string file)
        {
            ...
            var response = await _webDavClient.GetRawFile(_serviceSettings.NextCloudServer + file);
        ...
        public async Task<FileUploadStatusDto> UploadFile(IFormFile file)
        {
            ...
            try
            {
                ...
                await _webDavClient.PutFile(_serviceSettings.NextCloudServer + _serviceSettings.NextCloudBasePath + fileName, fileStream);
            }
            catch(Exception ex)
            {
                dto.HasErrors = true;
        ...
        public async Task<FileUploadStatusDto> UploadFile(IFormFile file, string targetDirectory)
        {
            ...
            try
            {
                ...
                await _webDavClient.PutFile(_serviceSettings.NextCloudServer + targetDirectory + fileName, fileStream);
            }
            catch (Exception ex)
            {
                dto.HasErrors = true;
        ...
        public async Task<FileDeletedDto> DeleteFile(string file)
        {
            ...
            try
            {
                await _webDavClient.Delete(_serviceSettings.NextCloudServer + file);
                dto.IsDeleted = true;
            }
            catch(Exception ex)
            {
        ...
        private async Task<List<string>> GetDirectories(string path)
        {
            ...
            var response = await _webDavClient.Propfind(_serviceSettings.NextCloudServer + path);
            ...
            if (response.IsSuccessful)
            {
                ...
                foreach (var resource in resources)
                {
                    ...
                    {
                        directories.Add(resource.Uri.ToString().Replace(_serviceSettings.NextCloudStripPart, ""));
                    }
                }
            }
        ...
        private async Task<List<string>> GetFiles(string path)
        {
            ...
            if (response.IsSuccessful)
            {
                ...
                foreach (var resource in resources)
                {
                    if (!resource.IsCollection)
                    {
                        files.Add(resource.Uri.ToString().Replace(_serviceSettings.NextCloudStripPart, ""));
                    }
                }
            }
